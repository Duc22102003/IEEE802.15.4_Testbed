...
#include "sl_simple_button_BUT_config.h"

sl_simple_button_context_t simple_but_context = {
  .state = 0,
  .history = 0,
  .port = SL_SIMPLE_BUTTON_BUT_PORT,
  .pin = SL_SIMPLE_BUTTON_BUT_PIN,
  .mode = SL_SIMPLE_BUTTON_BUT_MODE,
};

const sl_button_t sl_button_but = {
  .context = &simple_but_context,
...
{
  sl_button_init(&sl_button_but);
}

void sl_simple_button_poll_instances(void)
{
  sl_button_poll_step(&sl_button_but);
}
